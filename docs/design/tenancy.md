...
We should move issue [pipeline generation](https://github.com/ashutoshojha5/bytebase/blob/main/frontend/src/plugins/issue/DatabaseSchemaUpdateTemplate.ts#L17) from frontend to [backend](https://github.com/ashutoshojha5/bytebase/blob/main/server/issue.go#L406). We'd like the backend to take more responsibility for complexity. This also allows the backend to figure out the pipeline based on project tenant mode and deployment configuration.
...
#### Tenant Mode
This will be a new ENUM field on the existing [project table](https://github.com/ashutoshojha5/bytebase/blob/main/store/migration/10001##init_schema.sql#L202).
